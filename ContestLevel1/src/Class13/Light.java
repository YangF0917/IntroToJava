...
public class Light
{
	...
	// color to "white".
	 * @param o the on/off state of the bulb
	 * @param b the bulb state
	 * @param c the color of the light produced
	 */
	public Light(boolean o, boolean b, String c)
	{
		...
		burntOut = b;
		if (b) on = false;
		if (c.equalsIgnoreCase("red") || c.equalsIgnoreCase("green") || c.equalsIgnoreCase("blue")) color = c.toLowerCase();
		else color = "white";
	}
	...
	{
		String powerState, bulbState;
		if (on) powerState = "on";
		else powerState = "off";
		if (burntOut) bulbState = "burnt out";
		else bulbState = "not burnt out";
		return powerState + " " + color + "\t" + bulbState;
	}
	...
	public void flip()
	{
		if (!burntOut && !on) on = true;
		else if (burntOut) on = false;
		else on = false;
	}
